...
- **Redis 为什么快**：数据主要在内存中，数据结构和命令路径较短；事件循环与 I/O 多路复用减少线程切换和阻塞等待。但网络、命令复杂度、大 Key、持久化和单线程长命令仍会成为瓶颈。
- **过期删除**：过期时间是 Key 的有效期；访问时可惰性检查，后台也会主动抽查清理，因此“到点删除”不等于精确瞬时删除。
- **内存淘汰**：达到 `maxmemory` 后按配置策略淘汰 Key，例如不淘汰、按 TTL、近似 LRU/LFU 或随机淘汰；淘汰策略必须和缓存是否允许丢失相匹配。
- **BigKey**：单 Key 的成员过多或 Value 过大，会放大读写、序列化、网络和删除成本；应拆分、分批访问和渐进删除，避免大范围阻塞命令。
- **HotKey**：少数 Key 被高频访问，会造成单节点 CPU、网络或线程处理压力；可用本地缓存、读副本、分片 Key、限流或热点隔离缓解，具体方案要结合一致性要求。
- **Cluster 基本作用**：通过分片把 Key 分布到多个节点，扩展容量和吞吐，并提供节点故障处理能力；跨 slot 多 Key 操作、事务和 Lua 有约束，不能把 Cluster 等同于强一致或零故障。
